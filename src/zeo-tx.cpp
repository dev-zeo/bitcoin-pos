...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " zeo-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  zeo-tx [options] <hex-tx> [commands]  Update hex-encoded zeo transaction\n" +
            "or:     zeo-tx [options] -create [commands]   Create hex-encoded zeo transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded zeo transaction
