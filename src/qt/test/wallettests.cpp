...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<ZeoAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_zeo-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_zeo-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_zeo-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = ZeoUnits::formatWithUnit(unit, balance, false, ZeoUnits::separatorAlways);
    ...
    // Amount input
    ZeoAmountField* amountInput = receiveCoinsDialog.findChild<ZeoAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: zeo:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_zeo-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
