// Copyright (c) 2018 The coinBit Core developers
...
// zeo includes.
#include <..\src\script\zeoconsensus.h>
...
int main()
{
    std::cout << "zeoconsensus version: " << zeoconsensus_version() << std::endl;
    ...
    auto op0Result = zeoconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, zeoconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
