Zeo Core version 0.19.0.1 is now available from:

  <https://zeocore.org/bin/zeo-core-0.19.0.1/>
...
Zeo Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Zeo Core on unsupported systems.
...
to open the Zeo Core .dmg. This is due to new signing requirements
imposed by Apple, which the Zeo Core project does not yet adhere too.
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Zeo
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `zeod -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Zeo Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Zeo Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Zeo Core. (#14054)

- To eliminate well-known denial-of-service vectors in Zeo Core,
  ...
  This prevents Zeo Core from sending the BIP111 NODE_BLOOM service
  ...
  of Zeo Core and nodes that have manually enabled BIP37 support,
...
- By default, Zeo Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `zeo-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Zeo Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Zeo Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Zeo Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Zeo address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  ZEO), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in ZEO per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Zeo Core v0.17.0), this can eliminate a
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (zeoVBR)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/zeo/zeo/).
